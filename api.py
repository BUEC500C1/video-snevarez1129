...
def videoProcessor(handles, ffm):
    while(True):
        ...
        if twitter_handle is not None: #if there is a twitter handle in the queue
            imgCount = len(glob.glob1(r"imageFiles/", twitter_handle + r"*.png")) #check how many images are in the folder
        ...
        handles.task_done() #indicate that the video for this twitter handle has been created
...
def tweetsToPics(tweets, f):
    while(True):
        ...
        if nextTweet is not None: #if there is a tweet to convert
            f.createImage(nextTweet[0], nextTweet[1], nextTweet[2], nextTweet[3]) #convert tweet to an image; 0=handle, 1=profile pic, 2=tweet, 3=count
        tweets.task_done() #unblock join call from addTweets() once all tweets have been turned to image frames
        time.sleep(0.5) #sleep for half a second
...
class Video(Resource):
    ...
    def get(self, twitter_handle):
        ...
        try:
            ...
            s = stubs.stubFuncs() #create an object for stubs, these will run if no keys are available
            resp = s.noKeys()
            return resp
